...
export default function TeacherContentPage() {
  ...
  const [modalType, setModalType] = useState<'chapter' | 'lo'>('chapter');
  ...
  // Teacher only sees subjects assigned to them
  const teacherSubjects = subjects.filter(s => currentUser?.subjectIds.includes(s.id));
  ...
  const openModal = (type: 'chapter' | 'lo', id: string | null = null) => {
    setModalType(type);
    setEditingId(id);
    if (id) {
      let item: any;
    ...
    <DashboardLayout requiredRole={UserRole.TEACHER}>
      <div className="space-y-8">
        <div>
          ...
          <p className="text-gray-600">Manage Chapters and Learning Objectives for your assigned subjects</p>
        </div>
          ...
          <div className="card h-[calc(100vh-250px)] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto space-y-2">
              {teacherSubjects.length === 0 ? (
                <p className="text-gray-400 text-sm text-center mt-10">No subjects assigned yet. Contact Admin.</p>
              ) : (
                teacherSubjects.map(subject => (
                  <div 
                    ...
                    className={`p-3 rounded-lg cursor-pointer transition flex justify-between items-center ${selectedSubjectId === subject.id ? 'bg-primary-50 border-primary-200 border' : 'hover:bg-gray-50 border border-transparent'}`}
                  >
                    <div>
                      <span className="font-medium block">{subject.name}</span>
                      <span className="text-xs text-gray-400">{grades.find(g => g.id === subject.gradeId)?.name}</span>
                    </div>
                  </div>
                ))
              )}
              ...
              {!selectedSubjectId ? (
                ...
                filteredChapters.map(chapter => (
                  <div 
                    ...
                    className={`p-3 rounded-lg cursor-pointer transition flex justify-between items-center ${selectedChapterId === chapter.id ? 'bg-primary-50 border-primary-200 border' : 'hover:bg-gray-50 border border-transparent'}`}
                  >
